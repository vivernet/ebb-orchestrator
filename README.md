...
# Web UI (frontend) — в отдельном терминале
pnpm web:dev
```

Или используйте root скрипты:
...
Сервер по умолчанию слушает `http://127.0.0.1:3000`. Web UI запускается через Vite dev server (порт по умолчанию 5173, может быть другим если порт занят).

...
export class ProjectProjection {
  ...
  get(id: string): ProjectOverviewProjection | undefined {
    ...
    const epics = this.db.all<WorkRow>("SELECT id,display_id,title,status,contract_json FROM epics WHERE project_id=$id ORDER BY created_at", { id });
    const tasks = this.db.all<WorkRow>("SELECT id,epic_id,display_id,title,status,required,contract_json FROM tasks WHERE project_id=$id ORDER BY created_at", { id });

...
export class ProjectProjection {
  ...
  get(id: string): ProjectOverviewProjection | undefined {
    ...
    return { project: { id: p.id, name: p.name, displayName: p.display_name, status: p.status }, git: { repositoryPath: git?.repository_path ?? null, branch: git?.branch_name ?? null, defaultBranch: git?.target_ref ?? null, github: null, worktreePath: null }, epics, tasks, approvals, blockers, events, usage };
  }
